...
5. latest_by_offset(col1) group by col은 group by절 col으로 가장 최근 offset을 가지는 col1 값을 추출. 반대로 earliest_by_offset(col) group by col은 group by 절 col으로 가장 처음 offset을 가지는 col1값을 추출.

```sql
ksql> select customer_id
           , latest_by_offset(activity_type) as latest_act_type
        from customer_activity_stream
...
ksql> select customer_id
           , earliest_by_offset(activity_type) as earliest_act_type
        from customer_activity_stream
       group by customer_id
        emit changes;
...
22. Group by CTAS를 이용하여 Table기반의 MView를 만들때 Group by 절에 기술된 컬럼들은 자동으로 MView의 PK가 됨.  이때 Group by 절에 여러개의 컬럼들이 있을 경우 KEY_FORMAT을 KAFKA가 아닌 JSON또는 AVRO로 지정해 줘야함. 여러개의 PK 컬럼들은 KAFKA와 같은 Primitive 타입이 될 수 없기 때문임. 따라서 CTAS의 WITH절에 KEY_FORMAT=’JSON’과 같이 명시적으로 지정을 해줘야 함.
