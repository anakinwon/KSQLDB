...
```sql
DELCON dgen_shoe_customers_avro;
DELCON dgen_shoes_avro;
DELCON dgen_shoe_orders_avro;
DELCON dgen_shoe_clickstream_avro;
DELCON es_sink_order_metric;
DELCON es_sink_order_metric_per_mins;
...
RGJS.sh dgen_shoe_avro.json
RGJS.sh dgen_shoe_customers_avro.json
RGJS.sh dgen_shoe_orders_avro.json
RGJS.sh dgen_shoe_clickstream_avro.json
RGJS.sh es_sink_order_metric.json
RGJS.sh es_sink_order_metric_per_mins.json




- Shoes 상품 데이터를 랜덤 생성하는 Connector 생성. 
- dgen_shoe_avro.json 에 저장.
...
- Shoes  Customer 데이터를 랜덤 생성하는 Connector 생성. 
- dgen_shoe_customers_avro.json 에 저장
...
- Shoes  Orders 데이터를 랜덤 생성하는 Connector 생성. 
- dgen_shoe_orders_avro.json 에 저장.
...
- Shoes Clickstream 데이터를 랜덤 생성하는 Connector 생성.  
- dgen_shoe_clickstream_avro.json 에 저장.
...
- 고객 별 주문 건수. 고객명별로 주문 건수 구하기. 
- 먼저 shoe_orders와 shoes, shoe_customers 를 조인하여 
- shoe_orders의 고객과 상품에 대한 추가 속성을 가지는 Join Stream을 생성.
...
select a.order_id
     ...
     , 1 as dummy
     , a.ts as ts
     , from_unixtime(a.ts) as ts_format
  from shoe_orders a
...
as
select grade
     , count(*) as customer_grade_cnt
     , as_value(grade) as grade_val
  from grade_by_customer
 group by grade emit changes;
...
select state
     , as_value(state) as state_val
     , from_unixtime(windowstart) as w_start
     , from_unixtime(windowend) as w_end
     , count(*) as order_cnt
     , avg(sale_price) as avg_amount
     , sum(sale_price) as total_amount
...
select brand
     ...
     , from_unixtime(windowend) as w_end
     , count(*) as page_cnt
     , topk(sale_price, 5) as top5_prices
  from shoe_clickstream_enriched a
window tumbling (size 30 minutes)
 group by brand
  emit changes;
...
- 
```sql
SELECT from_unixtime(1611109800000)
  FROM shoes 
